...
//        };
        ...
        $(btnExit).click(function () {
            $.easyui.messager.confirm("系统提示", "您确定要退出系统并关闭该网页？", function (c) {
                if (c) { 
                	$.ajax({
    					async : false,
    					cache : false,
...
//                	$.util.closeWindowNoConfirm(); 
    ...
    //自己添加的，点击menu按钮，添加tab
    window.mainpage.clickMenuAddModelTab = function (menuId,menuText,menuPid,url,icon,iniframe) {
    	// {id: "1101", text: "jQuery jDirk", pid: "11", attributes: {href: "examples/example.html?path=jdirk/demo.html"}, state: "open"}
    	var node = {
    		id:menuId,
    		text:menuText,
    		pid:menuPid,
    		domId:"main_menu_"+menuId,
    		iconCls:icon,
    		attributes:{
    			href:url,
    			iniframe:iniframe,
    ...
    window.mainpage.mainTabs.addModule = function (title, href, iconCls, iniframe, closable, refreshable, selected) {
        var opts = window.mainpage.mainTabs.parseCreateTabArgs(arguments), 
            isExists = window.mainpage.mainTabs.isExists(opts.title, opts.href);
        console.log(opts);
        switch (isExists) {
